...
export const assetFromJSON = (asset: any): PacificAsset => {
    ...
    const fromJSON: PacificAsset = {
        ...
        collections: collectionFromJSON(asset.collection),
        // orders: asset.orders ? asset.orders.map(orderFromJSON) : undefined,
        // sellOrders: asset.sell_orders ? asset.sell_orders.map(orderFromJSON) : undefined,
        // buyOrders: asset.buy_orders ? asset.buy_orders.map(orderFromJSON) : undefined,
        ...
        lastSale: asset.last_sale ? assetEventFromJSON(asset.last_sale) : undefined,
        backgroundColor: asset.background_color ? `#${asset.background_color}` : undefined,
...
export const assetEventFromJSON = (assetEvent: any): AssetEvent => {
    return {
        ...
        transaction: assetEvent.transaction ? transactionFromJSON(assetEvent.transaction) : undefined,
        paymentToken: assetEvent.payment_token ? tokenFromJSON(assetEvent.payment_token) : undefined,
    }
}
...
export const accountFromJSON = (account: any): PacificAccount => {
    return {
        address: account.address,
        config: account.config,
        profileImgUrl: account.profile_img_url,
        user: account.user ? userFromJSON(account.user) : undefined
    }
}

export const userFromJSON = (user: any): string => {
    return user.username
}

export const assetBundleFromJSON = (asset_bundle: any): PacificAssetBundle => {

    const fromJSON: PacificAssetBundle = {
        ...
        // sellOrders: asset_bundle.sell_orders ? asset_bundle.sell_orders.map(orderFromJSON) : undefined
    }

    return fromJSON
}

export const assetContractFromJSON = (asset_contract: any): PacificAssetContract => {
    return {
...
export const collectionFromJSON = (collection: any): PacificCollection => {
...
export const tokenFromJSON = (token: any): PacificFungibleToken => {

    const fromJSON: PacificFungibleToken = {
        name: token.name,
        symbol: token.symbol,
...
export const orderFromJSON = (order: any): Order => {
    ...
    const fromJSON: Order = {
        ...
        makerAccount: accountFromJSON(order.maker),
        takerAccount: accountFromJSON(order.taker),
        ...
        createdTime: (Math.round(createdDate.getTime() / 1000)).toString(),
